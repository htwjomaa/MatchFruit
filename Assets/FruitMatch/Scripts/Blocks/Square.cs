    ...
    public class Square : MonoBehaviour, ISquareItemCommon, IField, IMarmaladeTargetable
    {
        ...
        public Item Item
        {
            ...
            set
            {
                ...
                if(item != null)
                {
                    if (GetSubSquare().aboveItem && (item.GetSpriteRenderer().sortingOrder >= GetSpriteRenderer().sortingOrder || subSquares.Count>1 && subSquares[subSquares
                    .Count-2].GetComponent<SpriteRenderer>().sortingOrder >= item.GetSpriteRenderer().sortingOrder))
                    {
        ...
        /// teleport destination position
         public Vector2Int teleportDestinationCoord;
        ...
        //     {
        //         timer = 0.3f;
        //         if (item == null && isEnterPoint && LevelManager.GetGameStatus() != GameState.RegenLevel && _spriteRenderer.enabled) StartCoroutine(CheckNullItem());
        //     }
        //    
        // }
...
//            itemComponent.StartFalling();
            switch (itemType)
            {
                case ItemsTypes.MARMALADE:
                    foreach (var marmaladeFly in itemComponent.GetComponentsInChildren<SpriteRenderer>())
                    {
              
                        //marmaladeFly.sprite = (Sprite)GenericSettingsFunctions.GetCopy(LoadingManager.loadedMarmaladeSprites[itemComponent.color]);
                    }
                    break;
                case ItemsTypes.VERTICAL_STRIPED:
                    foreach (var marmaladeFly in itemComponent.GetComponentsInChildren<SpriteRenderer>())
                    {
                        //marmaladeFly.sprite = LoadingManager.loadedMarmaladeSprites[itemComponent.color];
                    }
                    break;
        ...
        /// </summary>
        private IEnumerator DelayTeleportSetCo(float sec)
        {
...
//        if (!nextSq?.IsNone() ?? false)
        ...
        public void SetType(SquareTypes _type, int sqLayer, SquareTypes obstacleType, int obLayer)
        {
            ...
            if (type != _type && !IsTypeExist(_type) && IsAvailable())
            {
                for (var i = 0; i < sqLayer; i++)
                {
                    ...
                    if (prefab != null && _type != SquareTypes.EmptySquare && _type != SquareTypes.NONE)
                    {
                        ...
                        if (_type != SquareTypes.JellyBlock)
                            subSquares.Add(square);
                        else
                            subSquares.Insert(0, square);
        ...
        public void DestroyBlock(bool destroyTarget=false)
        {
            ...
            if (subSquares.Count > 0)
            {
                ...
                if (subSquare.GetComponent<TargetComponent>() == null)
                {
                    ...
                    Rl.GameManager.PlayAudio(Rl.soundStrings.DestroyBlockAudio, Random.Range(0,5),true, Rl.settings.GetSFXVolume, Rl.effects.audioSource);
        ...
        public bool IsCleared()
        {
            ...
            var isCleared = enumerable.All(i=>i.subSquares.Last()==this);
            return  isCleared;
        }
        ...
        {
            var obj = GetBlockPrefab(sqType)?.First();
    ...
    public class Texture2DSize
    {
        ...
        public Texture2DSize(Texture2D texture2D, Vector2Int v, int _order, bool rotate = false)
        {
            Texture2D = texture2D;
            size = v;
            order = _order;
