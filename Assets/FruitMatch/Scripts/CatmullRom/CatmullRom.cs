//----------------  CatMullRom Splines----------------//
// Calculates Spline points on given Controlpoints
// Feed in an array of controlpoints and set resolution + loop/ no loop
// and the Algorithm will calculate the points for you.
// It is partly multithreaded but still needs improvement
// Algorithm: (2t^3 - 3t^2 + 1) * p0
//            + (t^3 - 2t^2 + t) * tan0
//            + (-2t^3 + 3t^2) * p1
//            + (t^3 - t^2) * tan1
//----------------                    -------------------//
...
public sealed class CatmullRom
    {
        ...
        //
        // }
        public static CatmullRomPoint GetPointFromT(float t, CatmullRomPoint[] catmullRomPoints)
        {
           int n = (int)MathLibrary.Remap(0, 1, 1, catmullRomPoints.Length - 1, t);
           return catmullRomPoints[n];
        }
