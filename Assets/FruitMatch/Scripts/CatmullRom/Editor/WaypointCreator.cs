...
//----------------  Waypoint Creator----------------//
// can easily create Waypoints if you feed in a parent with the right script
// Undo System is done but it still needs an upgrade to not draw waypoints
//based on current and next node but on an array Index (easy to fix)
//----------------                    -------------------//

using UnityEditor;
using UnityEngine;
public sealed class WaypointCreator : EditorWindow
{
#if UNITY_EDITOR
 [UnityEditor.MenuItem("FruitMatch/Waypoint Creator")]
