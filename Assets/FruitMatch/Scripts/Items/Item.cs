...
//types of items
    ...
    public class Item : ItemMonoBehaviour, ISquareItemCommon, IField, IColorChangable, IMarmaladeTargetable
    {
        ...
        private void OnEnable()
        {
            ...
            InitItem();
            if ( square != null && square.item != null && square.Item.gameObject != gameObject) Destroy(gameObject);
        }
...
//        
                    Rl.GameManager.PlayAudio(Rl.soundStrings.SwapBackAudio, Random.Range(0,5), Rl.settings.GetSFXVolume, Rl.effects.audioSource);
                }
                else
                {
        ...
        {
            return Rl.Cam.ScreenToWorldPoint(Input.mousePosition);
        }
        /// <summary>
...
//        transform.position = destPos;// square.transform.position;
            ...
            {
                Rl.GameManager.PlayAudio(Rl.soundStrings.DropClips[Random.Range(0, Rl.soundStrings.DropClips.Count)],
                    Random.Range(0, 5),  Rl.settings.GetSFXVolume, Rl.effects.audioSource);
            }
        ...
        IEnumerator ChangeTypeCor(Action<Item> callback=null,bool dontDestroyForThisCombine = true, bool destroyBlock = true)
        {
            ...
            if(LevelManager.GetGameStatus() != GameState.PrepareGame)
                Rl.GameManager.PlayLimitSound(Rl.soundStrings.AppearStripedColorBombSound,
                    Random.Range(0, 5),  Rl.settings.GetSFXVolume, Rl.effects.audioSource);
            
            Item newItem = square.GenItem(false, NextType, LoadingHelper.THIS.ColorHelper(color));
        ...
        [SerializeField] [HideInInspector] private FieldBoard _field;
        ...
        private IEnumerator DestroyCor(bool showScore = false, bool particles = true, Item explodedItem = null, bool explEffect = false)
        {
            ...
            Rl.GameManager.PlayAudio(Rl.soundStrings.DestroyClips[Random.Range(0, Rl.soundStrings.DestroyClips.Count)],
                Random.Range(0, 5),  Rl.settings.GetSFXVolume, Rl.effects.audioSource);
        ...
        public bool IsBottom() => square.IsBottom();
    }
    [Serializable]
