...
//                SetAnimationKeys();
        ...
        {
            bool wrongSprite = false;
            for (int i = 0; i < LoadingManager.loadedSprites.Length; i++)
            {
                if (sprite == LoadingManager.loadedSprites[i])
                    wrongSprite = true;
            }
            return wrongSprite;
        }
        ...
        private static bool ArgItems(Item i) => i.Explodable && !i.needFall && !i.falling && !i.destroying && !i.JustCreatedItem;

        public bool IsAnimationFinished() => reachedTarget;

        public int GetPriority() => priority;

        public bool CanBeStarted() => canBeStarted;
    }
    ...
    public static class MarmaladeUtils
    {
        public static IEnumerable<IMarmaladeTargetable> MarmaladeCondition(this IEnumerable<IMarmaladeTargetable> seq, GameObject gameObject, Item item)
            => seq.WhereNotNull().Where(i => (i.GetMarmaladeTarget == null || i.GetMarmaladeTarget == gameObject)&&i != item && (!i.GetItem?.destroying ?? true));
    }
}