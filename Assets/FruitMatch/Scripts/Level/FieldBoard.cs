...
using Random = UnityEngine.Random;
 ...
 namespace FruitMatch.Scripts.Level
 {
     ...
     public class FieldBoard : MonoBehaviour
     {
         ...
         {
             GameField = transform;
            // Invoke(nameof(InvokeSideDot), 0.5f);
         ...
         IEnumerator DelaySetTeleCo(float sec)
         {
             yield return new WaitForSeconds(sec);
             SetTeleToField();
         }
         public void SetTeleToField()
         {
             foreach (Square i in squaresArray.ToList())
             { 
         ...
         public void CreateField()
         {
             ...
             for (int row = 0; row < fieldData.maxRows; row++)
             {
                 ...
                 {
                     SquareBlocks squareBlock = fieldData.levelSquares[row * fieldData.maxCols + col];
            ...
            // StartCoroutine(DelaySetTeleCo(0.3f));
             foreach (Square i in squaresArray.ToList())
             { 
                 i.SetBorderDirection();
                 i.SetTeleports();  
...
//            yield return new WaitForEndOfFrame();
         ...
         {
             bool prepareLevel = !falling;
             Square[] squares = squaresArray;
             if (fieldData.levelSquares.Any(i=>i.item.ItemType == ItemsTypes.INGREDIENT)) IngredientsByEditor = true;
         ...
         private void CreateSquare(int col, int row, bool chessColor = false)
         {
             ...
             Array.Resize(ref fieldData.levelSquares, fieldData.maxCols*fieldData.maxRows);
             SquareBlocks squareBlock = fieldData.levelSquares[row * fieldData.maxCols + col];
