    ...
    private void GetSearchResults()
    {
        ...
        foreach (KeyValuePair<string, string> element in dictionary)
        {
            if (element.Key.ToLower().Contains(value.ToLower()) || element.Value.ToLower().Contains(value.ToLower()))
            {
                ...
                if (GUILayout.Button(content, GUILayout.Height(20), GUILayout.Width(20)))
                {
                    if (EditorUtility.DisplayDialog("Remove Key" + element.Key + "?",
                            "This will remove the element from the localisation, are you sure?", "Do it"))
                    {
                ...
                //another button for editing values in this list
                EditorGUILayout.EndHorizontal();
            }
        }
