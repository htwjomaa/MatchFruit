    ...
    public class TargetCounter
    {
        ...
        private int GetCountForSquares(bool subLevel)
        {
            if (targetLevel != null && targetLevel.setCount == TargetSystem.SetCount.FromLevel)
            {
                ...
                {
                    count = GetSquareTargetCount(subLevel);
                    return count;
                }
        ...
        public void BindGUI(TargetGUI targetGui)
        {
            ...
            if (targetLevel.name == "Stars" && !showScores)
            {
                ...
                LeanTween.delayedCall(star, 5f, () => { LeanTween.scale(star, Vector3.one * 1.2f, 0.4f).setLoopPingPong().setRepeat(4); }).setRepeat(-1);
            }
        }
        ...
        public void CheckTarget(Square[] squares, bool afterDestroy=true)
        {
            ...
            {
                var obj = Object.FindObjectsOfType<TargetComponent>().Where(i => extraObjects.Any(x => x == i.GetComponent<SpriteRenderer>().sprite));
                ...
                foreach (var item in squares)
                {
                    if(item != null && item.gameObject != null && item.GetSprite() != null)OnCheckTarget(item.gameObject, item.GetSprite());
                }
